...
export class SiteQueryComponent implements OnInit {
  ...
  private vocsToQuery: Vocabulary[] = [{primaryLanguage: "", tries:0, clas:"", unit:"", secondaryLanguage: "", failuresCount:0}];
  ...
  constructor(public vocService: VocabularyService, public router: Router, public route: ActivatedRoute, public dialog: MatDialog) { 
    ...
    if (this.unit === undefined || this.unit === null) {
      ...
      vocService.getVocsFromOneClas(this.clas).then((result:Vocabulary[]) => {this.vocsToQuery = result; });
    } else {
      vocService.getVocsFromOneUnit(this.clas, this.unit).then((result:Vocabulary[]) => {this.vocsToQuery = result})
    }
  }
  ...
  checkPressed() {
    ...
    let secondaryLanguageInput = userInputField.innerText;
